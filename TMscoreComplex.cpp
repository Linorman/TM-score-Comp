...
/*****************************************************************/
// BEGIN CIF2PDB, Code is 
class Cif2PDB{
	public:
		Cif2PDB(const string& cif, const string& pdb):chainID_candidate("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-"){
		    string PDBtxt;
			cif2pdb(cif, PDBtxt);
			
			ofstream fout(pdb.c_str()); 
		    fout<<PDBtxt<<"END"<<endl;
		    fout.close();
		}
		
		void print_notice(){
		    cout << "* JUST WARNING *: Convert PDBx/mmCIF file PDBx.cif into PDB format file PDBx.pdb Since it is not always possible to convert chain ID from PDBx/mmCIF to PDB, the chain ID in coverted PDB file is systematically renamed.\n"
			    <<endl;
			exit(1);
		}
	private:
		const string chainID_candidate;
		...
		void cif2pdb(const string filename, string &PDBtxt){
		    bool loop_ = false; // not reading following content
		    map<string,int> _atom_site;
		    map<string,char> asym_id2chainID;
		    char chainID=' ';
		    int atom_site_pos;
		    ...
		    int filetype=0; // 0 - uncompressed; 1 - gz; 2 - bz2; -1 - stdin
		    ifstream fin;
		    if (filename=="-") filetype=-1;
		    else fin.open(filename.c_str());
		
		
		    while (filetype==-1?cin.good():fin.good())
		    {
		        if (filetype==-1)  getline(cin, line);
		        ...
		        if (group_PDB!="ATOM" && group_PDB!="HETATM") continue;
		        if (group_PDB=="ATOM") group_PDB="ATOM  ";
		        
		        /* remove alternatively located residue */
		        alt_id=".";
		        ...
		        {
		            if (model_index!="") PDBtxt+="ENDMDL\n";
		            model_index=line_vec[_atom_site["pdbx_PDB_model_num"]];
		            i8_stream<<"MODEL "<<setw(4)<<model_index<<'\n';
		            PDBtxt+=i8_stream.str();
		            i8_stream.str(string());
		
		            a=0;
		            asym_id2chainID.clear();
		        }
		
		        if (_atom_site.count("auth_asym_id"))
		             asym_id=line_vec[_atom_site["auth_asym_id"]];
		        else asym_id=line_vec[_atom_site["label_asym_id"]];
		        if (asym_id2chainID.count(asym_id)==0)
		        {
		            chainID=chainID_candidate[
		                asym_id2chainID.size() % chainID_candidate.size()];
		            asym_id2chainID[asym_id]=chainID;
		        }
		        ...
		        Cartn_x=atof(line_vec[_atom_site["Cartn_x"]].c_str());
		        Cartn_y=atof(line_vec[_atom_site["Cartn_y"]].c_str());
		        Cartn_z=atof(line_vec[_atom_site["Cartn_z"]].c_str());
		
		        B_iso_or_equiv=0;
		        if (_atom_site.count("B_iso_or_equiv")) B_iso_or_equiv=
		            atof(line_vec[_atom_site["B_iso_or_equiv"]].c_str());
		        
		        a++;
		        i8_stream<<group_PDB
		            <<setw(5)<<a % 100000<<" "<<atom<<" "<<AA<<" "
		            <<asym_id2chainID[asym_id]<<setw(5)<<resi<<"   "
		            <<setiosflags(ios::fixed)<<setprecision(3)
		            <<setw(8)<<Cartn_x<<setw(8)<<Cartn_y<<setw(8)<<Cartn_z
		            <<"  1.00"<<setiosflags(ios::fixed)<<setprecision(2)
		            <<setw(6)<<B_iso_or_equiv;
		        if (_atom_site.count("type_symbol"))
		            i8_stream<<setw(12)<<line_vec[_atom_site["type_symbol"]];
		        PDBtxt+=i8_stream.str()+"\n";
		        i8_stream.str(string());
		    }
		    if (model_index!="") PDBtxt+="ENDMDL\n";
...
int main(int argc, char** args) {
	...
	for (i = 1; i < argc; i++){
		if (0 == strcmp(args[i], "-mol") && i < argc-1){
			...
			if (g_seqid_cutoff < 0.5) g_seqid_cutoff = 0.5;
			...
			exit(1);
		}else if (0 == strcmp(args[i], "-cif2pdb")){
			is_cif2pdb_func = true;
		}else{
			if (pdbFileInd == 0){
				isQueryProvide = true;
				query_pdb = args[i];
	...
	if (is_cif2pdb_func){
		if (!isQueryProvide){
			cout << "PLEASE PROVIDE PDB.cif FILE!!!" << endl << endl;
			CBaseFunc::print_help(args[0]);
		}else{
			fstream _file;
			...
			if(!_file){
				...
				cout << "Please check it and input a correct mmCIF/CIF file path!" << endl;
				exit(1);
			}
		} 
		if (!isTemplProvide){
			cout << "PLEASE PROVIDE PDB.pdb FILE!!!" << endl << endl;
			CBaseFunc::print_help(args[0]);
		}
		
		Cif2PDB cif2pdb(query_pdb, templ_pdb);
		cif2pdb.print_notice(); 
	}else{
		if (!isQueryProvide){
		...
		CTMscoreComplex* mtm = new CTMscoreComplex(query_pdb, templ_pdb, g_ali_type, chain_pair);
		mtm->print_result();
...
inline void CTMscoreComplex::align_monomer(const bool& fast){
	...
	if (g_use_res_orig_index_or_not){
		...
		if (LIGAND == qmt){
			...
			if (qseq != tseq){
				cout << "WRONG USAGE: the ligand atoms are not same." << endl;
				exit(1);
			}
...
inline void CNWalign::run_needleman_wunsch(){
	...
	int** score = CBaseFunc::new2DIntArr(alen, blen);
	int** val = CBaseFunc::new2DIntArr(alen+1, blen+1);
	int** idir = CBaseFunc::new2DIntArr(alen+1, blen+1);
	int** preV = CBaseFunc::new2DIntArr(alen+1, blen+1);
	int** preH = CBaseFunc::new2DIntArr(alen+1, blen+1);
	...
	delete[] seq1;
	delete[] seq2;
	CBaseFunc::delete2DIntArr(alen, score);
	CBaseFunc::delete2DIntArr(alen+1, val);
	CBaseFunc::delete2DIntArr(alen+1, idir);
	CBaseFunc::delete2DIntArr(alen+1, preV);
	CBaseFunc::delete2DIntArr(alen+1, preH);
...
inline double CBaseFunc::__2merExchangedSearch(double** scoMtx, const int& rownum, const int& colnum, int* alivec, int* transpose_alivec, const double& prev_score, const int& iter_max){
	...
	for (iter = 0; iter < iter_max; iter++){
		...
		if (fabs(score - prev_sco) < 1e-3) break; 
	}
...
inline void Complex::load(const string& pdb){
	int i, n;
	...
	while (getline(fin, line)) {
		...
		if (0 == line.compare(0, 4, "ATOM")) {
			...
			if (g_is_load_H2O || hetatm != "HOH"){
				chain_oindex = line.substr(21, 5);
			
				smit = small_molecule_chain_contents_map.find(chain_oindex);
				if (smit != small_molecule_chain_contents_map.end()){
					(smit->second)->push_back(line);
				}else{
					chain_contents = new vector<string>();
					chain_contents->push_back(line);
					small_molecule_chain_contents_map[chain_oindex] = chain_contents;
					
					ind_orders.push_back(chain_oindex);
					this->m_chains.push_back(CBaseFunc::stringTrim(hetatm) + "[" + CBaseFunc::stringTrim(chain_oindex) + "]");
				}
			}
		}
	}
	...
	n = ind_orders.size();
	for (i = 0; i < n; i++) {
		chain_oindex = ind_orders[i];
		chain_contents = small_molecule_chain_contents_map[chain_oindex];
...
inline void CTMscoreComplex::save_superposition_ditances(const string& savepath){
	...
	fout << "------------------------------------------------------------------" << endl;
	fout << "                 Query                              Templalte     " << endl;
	fout << "        ---------------------                ---------------------" << endl;
	fout << "ind.    chain    oind.   ant*   distance     chain    oind.    ant" << endl;  
	fout << "------------------------------------------------------------------" << endl;
	char buf[200];
	int i, n = this->aa_level_ali.size();
	for (i = 0; i < n; i++){
		ALIGN_PAIR ap = this->aa_level_ali[i];
		
		double dis = sqrt(ap.dis2);
		if (dis < 10.){
			sprintf(buf, "%4d    %5s    %5d    %3s    %8.6f    %5s    %5d    %3s", 
					i, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
		}else if (dis < 100){
			sprintf(buf, "%4d    %5s    %5d    %3s    %8.5f    %5s    %5d    %3s", 
					i, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
		}else if (dis < 1000){
			sprintf(buf, "%4d    %5s    %5d    %3s    %8.4f    %5s    %5d    %3s", 
					i, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
		}else if (dis < 10000){
			sprintf(buf, "%4d    %5s    %5d    %3s    %8.3f    %5s    %5d    %3s", 
					i, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
		}else if (dis < 100000){
			sprintf(buf, "%4d    %5s    %5d    %3s    %8.2f    %5s    %5d    %3s", 
					i, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
		}else if (dis < 1000000){
			sprintf(buf, "%4d    %5s    %5d    %3s    %8.1f    %5s    %5d    %3s", 
					i, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
		}
...
inline void CTMscoreComplex::print_result(){
	...
	cout << "Information of the complex in the 1st input: " << endl;
	if (0 != qpro_num){
		cout << "  >Protein number: " << qpro_num << ", protein chains: ";
		n = qpro_chains.size();
		for (i = 0; i < n-1; i++){
			cout << qpro_chains[i] << " (" << qpro_aanums[i] << " a.a.), ";
		}
		cout << qpro_chains[i] << " (" << qpro_aanums[i] << " a.a.)." << endl;
	} 
	if (0 != qdna_num){
		cout << "  >DNA number: "     << qdna_num  << ", DNA chains: ";
		n = qdna_chains.size();
		for (i = 0; i < n-1; i++){
			cout << qdna_chains[i] << " (" << qdna_nucnums[i] << " bp), ";
		}
		cout << qdna_chains[i] << " (" << qdna_nucnums[i] << " a.a.)." << endl;
	} 
	if (0 != qrna_num){
		cout << "  >RNA number: "     << qrna_num  << ", RNA chains: ";
		n = qrna_chains.size();
		for (i = 0; i < n-1; i++){
			cout << qrna_chains[i] << " (" << qrna_nucnums[i] << " nt), ";
		}
		cout << qrna_chains[i] << " (" << qrna_nucnums[i] << " a.a.)." << endl;
	}
	if (0 != qlig_num){
		cout << "  >Ligand number: "  << qlig_num  << ", Ligand types: ";
		n = qlig_types.size();
		for (i = 0; i < n-1; i++){
			cout << qlig_types[i] << " (" << qlig_atnums[i] << " atoms), ";
		}
		cout << qlig_types[n-1] << " (" << qlig_atnums[i] << " atoms)." << endl;
	}
	cout << endl;
	cout << "Information of the complex in the 2nd input: " << endl;
	if (0 != tpro_num){
		cout << "  >Protein number: " << tpro_num << ", protein chains: ";
		n = tpro_chains.size();
		for (i = 0; i < n-1; i++){
			cout << tpro_chains[i] << " (" << tpro_aanums[i] << " a.a.), ";
		}
		cout << tpro_chains[i] << " (" << tpro_aanums[i] << " a.a.)." << endl;
	} 
	if (0 != tdna_num){
		cout << "  >DNA number: "     << tdna_num  << ", DNA chains: ";
		n = tdna_chains.size();
		for (i = 0; i < n-1; i++){
			cout << tdna_chains[i] << " (" << tdna_nucnums[i] << " bp), ";
		}
		cout << tdna_chains[i] << " (" << tdna_nucnums[i] << " a.a.)." << endl;
	} 
	if (0 != trna_num){
		cout << "  >RNA number: "     << trna_num  << ", RNA chains: ";
		n = trna_chains.size();
		for (i = 0; i < n-1; i++){
			cout << trna_chains[i] << " (" << trna_nucnums[i] << " nt), ";
		}
		cout << trna_chains[i] << " (" << trna_nucnums[i] << " a.a.)." << endl;
	}
	if (0 != tlig_num){
		cout << "  >Ligand number: "  << tlig_num  << ", Ligand types: ";
		n = tlig_types.size();
		for (i = 0; i < n-1; i++){
			cout << tlig_types[i] << " (" << tlig_atnums[i] << " atoms), ";
		}
		cout << tlig_types[n-1] << " (" << tlig_atnums[i] << " atoms)." << endl;
	}
	cout << endl;
	
	cout << "Chain alignment information: " << endl;
	cout << "  >The 1st input: ";
	for (i = 0; i < qsize; i++){
		if (-1 != obj_level_ali[i]){
	...
	for (i = 0; i < qsize; i++){
		if (-1 != obj_level_ali[i]){
			cout << this->templ->get_chain( obj_level_ali[i]) << ' ';
		}
	}
	cout << endl;
	...
	for (i = 0; i < qsize; i++){
		if (-1 != obj_level_ali[i]){
			...
			sprintf(buf, "  >TM-score between chain %s(1st input) and %s(2nd input): %8.6f", query->get_chain(i).c_str(), templ->get_chain( obj_level_ali[i]).c_str(), tmsco/num);
			cout << buf << endl; 
	...
	for (i = 0; i < n; i++){
		ALIGN_PAIR ap = this->aa_level_ali[i];
		dis2.push_back(ap.dis2);
	}
	double rmsd = CBaseFunc::rmsd(dis2);
	double gdt_ts = CBaseFunc::gdt_ts(dis2);
	double gdt_ha = CBaseFunc::gdt_ha(dis2);
	
	sprintf(buf, "rTM-score(%d): %8.6f, rTM-score: %8.6f, TM-score: %8.6f", max_rTM_level, corr_rTM_of_max_rTM_level, rtmscore, tmscore);
	cout << buf << endl;
	sprintf(buf, "RMSD: %8.6f, GDT_TS: %8.6f, GDT_HA: %8.6f", rmsd, gdt_ts, gdt_ha);
	cout << buf << endl;
	cout << endl;
	cout << "Rotation Matrix (rotate the 2nd input to the 1st input):" << endl;
	vector<string> inv_ut_vec = formatInvRotMtx();
	n = inv_ut_vec.size();
	for (i = 0; i < n; i++)
	...
	if (g_is_output_in_detail){
		...
		if (0 == qlig_num || 0 == tlig_num){
			cout << "------------------------------------------------------------------" << endl;
			cout << "                 Query                              Templalte     " << endl;
			cout << "        ---------------------                ---------------------" << endl;
			cout << "ind.    chain    oind.   ant*   distance     chain    oind.    ant" << endl;  
			cout << "------------------------------------------------------------------" << endl;
		}else{
			cout << "----------------------------------------------------------------------------" << endl;
			cout << "                  Query                                    Templalte        " << endl;
		...
		for (i = 0; i < n; i++){
			...
			if (0 == qlig_num || 0 == tlig_num){
				if (dis < 10.){
					sprintf(buf, "%4d    %5s    %5d    %3s    %8.6f    %5s    %5d    %3s", 
							i+1, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());
				}else if (dis < 100){
					sprintf(buf, "%4d    %5s    %5d    %3s    %8.5f    %5s    %5d    %3s", 
							i+1, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
				}else if (dis < 1000){
				...
				if (dis < 10.){
					sprintf(buf, "%4d    %10s    %5d    %3s    %8.6f    %10s    %5d    %3s", 
							i+1, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());
				}else if (dis < 100){
					sprintf(buf, "%4d    %10s    %5d    %3s    %8.5f    %10s    %5d    %3s", 
							i+1, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
				}else if (dis < 1000){
					sprintf(buf, "%4d    %10s    %5d    %3s    %8.4f    %10s    %5d    %3s", 
							i+1, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
				}else if (dis < 10000){
					sprintf(buf, "%4d    %10s    %5d    %3s    %8.3f    %10s    %5d    %3s", 
							i+1, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
				}else if (dis < 100000){
					sprintf(buf, "%4d    %10s    %5d    %3s    %8.2f    %10s    %5d    %3s", 
							i+1, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
				}else if (dis < 1000000){
					sprintf(buf, "%4d    %10s    %5d    %3s    %8.1f    %10s    %5d    %3s", 
							i+1, ap.qchain.c_str(), ap.qoind, ap.qaa.c_str(), dis, ap.tchain.c_str(), ap.toind, ap.taa.c_str());			
				}
			}
...
inline const bool LigAtomMatcher::is_same_important(const int& i, const LigAtomMatcher& other, const int& other_i){
	...
	return CVector::is_same_contents(ith, other_ith, true);
}
...
LigAtomMatcher::LigAtomMatcher(Molecule* p_lig):lig(*p_lig){
	...
	for (int i = 0; i < atnum; i++){
		for (int j = i+1; j < atnum; j++){
			double vdwij = 0.5*(vdwr[i] + vdwr[j]);
			double dis2 = CBaseFunc::distance2(lig[i], lig[j]);
			if (dis2 < vdwij*vdwij)
				adjmtx[i][j] = adjmtx[j][i] = 1.;
		}
	}
...
inline bool LigAtomMatcher::is_same_roads(const vector<vector<string>* >& arvec, const vector<vector<string>* >& brvec){
	return CVector::is_same_contents(arvec, brvec, true);
}
...
inline void CBaseFunc::print_help(const char* arg){
	...
	cout << " Usage: " << arg << " PDB1.pdb PDB2.pdb [Options]" << endl << endl
		 ...
		 << "             set the sequence identity cutoff (**default: 0.7**, range from 0.5 to" << endl
		 ...
		 << "               suf          : in average, the speed is the fastest one, the accuracy " << endl
		 << "                              is lower than that of 'fst'" << endl
		 << "               fst          : in average, the speed is the second fastest one, the " << endl
		 << "                              accuracy is lower than that of 'nml'" << endl 
		 << "               nml (default): in average, the speed is normal, the should be the best" << endl
